...
function ts_get_custom_sidebars_list($add_default = true) {
	...
	$sidebars = array();
	if ($add_default) {
		$sidebars['default'] = esc_html__('Default', 'splendid');
	}
 ...
 * Insert ajax icon
 */
function ts_ajax_insert_icon() {
 
	require get_template_directory().'/extensions/insert-icon.php';
	die();
}
 
add_action( 'wp_ajax_insert_icon', 'ts_ajax_insert_icon' );
...
add_action( 'admin_init', 'splendid_importer_init' );
add_action( 'wp_ajax_refresh_import_log', 'splendid_refresh_import_log' );
add_action( 'wp_ajax_reset_importer_status', 'splendid_reset_import_status' );

/**
 * Check if wordpress importer is activated
 * @return boolean
 */
function splendid_check_if_wordpress_importer_activated() {
 ...
 */
function splendid_importer_script() {
...
if (!splendid_check_if_wordpress_importer_activated()) {
	add_action('admin_enqueue_scripts', 'splendid_importer_script');
}

/**
 * Importer init
 */
function splendid_importer_init() {
	...
	if (isset($_GET['import_sample_data'])) {
		...
		if (wp_verify_nonce( $_GET['import_sample_data'], 'importer_sample_data' )) {		
			try {
				...
				$importer = new splendid_importer;
 ...
 * Refresh import log
 */
function splendid_refresh_import_log() {
 ...
 */
function splendid_reset_import_status() {
	delete_option('splendid_import_started');
	die();
}

class splendid_importer {
	...
	public function init($template = '') {
		...
		if (get_option('splendid_import_started') == 1) {
	...
	protected function import($file) {
		...
		update_option('splendid_import_started',1);
		$importer->import($xml);
		ob_end_clean();
	...
	protected function import_theme_options() {
		...
		$import_json = splendid_read_file( $this -> template_path, 'redux.json');
	...
	protected function import_widgets() {
		...
		$widget_data_json = splendid_read_file( $this -> template_path, 'widget_data.json');
	...
	public function log($message, $append = true) {
		...
		if (isset($upload_dir['baseurl'])) {
			...
			splendid_write_file($upload_dir['basedir'].'/', 'importer.log', $data, $append); 
		}
	}
	...
	public function get_log() {
		...
		if (isset($upload_dir['baseurl'])) {
			...
			if (file_exists($upload_dir['basedir'].'/importer.log')) {
				return splendid_read_file( $upload_dir['basedir'] . '/', 'importer.log');
			}
		}
		return '';
	}
